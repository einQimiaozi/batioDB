...
impl DBFile {
    pub fn new(path: String) -> Result<Self,&'static str> {
        let db_file = OpenOptions::new().append(true).read(true).open(path).expect("DBFile Error");
    ...
    pub fn read(&mut self, mut offset: u64) -> Entry {
        ...
        self.file.seek(SeekFrom::Start(offset + ENTRYHEADERSIZE as u64)).expect("error: offset invalid!");
